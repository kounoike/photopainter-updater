...
既存の HTTP サーバが `image.bmp` をそのまま返している構成を、`image.png` を入力として変換済み 24bit BMP を生成して返す構成へ拡張する。変換順序は、彩度を大きく強調し、`ref/convert.py` と同等のディザリング方針で量子化し、その結果を右 90 度回転した後に BMP として配信する。既存の取得先 `/` と `/image.bmp` は維持し、入力画像未配置や変換失敗時には入力画像由来の問題と判別できる失敗応答を返す。
...
**Testing**: Rust 自動テスト、参照変換との比較確認、HTTP 手動確認  
...
**Constraints**: 既存 route `/` と `/image.bmp` を維持する、入力画像は単一の `image.png`、参照変換と同等のディザリング方針を保つ、`firmware/` や `xiaozhi-esp32/` は変更しない  
...
**Structure Decision**: 実装は既存サーバの責務を保つため `server/` 配下へ閉じる。`server/src/main.rs` に画像読込、彩度変換、参照相当ディザリング、右 90 度回転、BMP 応答生成を統合し、`ref/convert.py` は品質基準として参照専用で扱う。新規 route は増やさず、既存の `/` と `/image.bmp` を維持する。
