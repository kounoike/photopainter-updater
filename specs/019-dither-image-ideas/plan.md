...
ePaper の表示特性、既存パレット、既存ディザリング前提を踏まえ、サーバー側の画像加工パイプラインに比較実験しやすい改善プロファイルを追加する。改善対象は前処理とディザリング処理の両方を含むが、HTTP エンドポイント、転送フォーマット、ファームウェアは変更しない。同じ入力画像を手動で差し替えながら必要時のみ追加画像で補足し、実機 ePaper 表示を主判定として比較結果を記録できるようにする。
...
**Storage**: ローカルファイル（`server/contents/` の手動差し替え入力画像、`server/testdata/`、`specs/019-dither-image-ideas/` の実験記録）  
**Testing**: `cargo test`、同じ入力画像の手動差し替え比較、実機 ePaper 表示確認  
...
- [x] 手動差し替え入力画像、実験プロファイル、比較記録により各 story の検証経路を定義している
...
**Structure Decision**: 実装変更は `server/` 配下に限定する。比較実験の設定追加は `server/src/config.rs` と `server/src/image_pipeline/` に寄せ、既存配信経路は `routes.rs` と `app.rs` の現行配線を維持する。比較に使う代表入力画像候補は既存 `server/testdata/` に保持し、実際の比較時は `server/contents/image.png` を手動差し替えで運用する。成果物としての比較観点と手順は `specs/019-dither-image-ideas/` に保持する。
...
比較時は同じ入力画像を手動で差し替えながら baseline と評価対象 profile を見比べ、必要な場合のみ追加画像を補足として使う。比較用の代表画像候補は `server/testdata/` に保持し、比較結果は profile ごとに同じ順序で確認する。
...
結果は実機 ePaper 表示を主判定として記録し、PC 上の BMP/Binary 出力やテスト結果は補助情報として使う。比較結果は profile、入力画像、観点、判断、次アクションの形で残す。
...
- `server/src/config.rs`: 改善 profile と split view 比較モード指定の設定追加
...
- `server/testdata/`: 代表入力画像候補の整備
- `specs/019-dither-image-ideas/`: 比較観点、実施手順、記録様式の文書化

## 現時点の整理

- 実機比較の現時点の上位候補は `color-priority + DITHER_DIFFUSION_RATE=0.8`
- `Atkinson` は黒粒低減には効くが、少なくともイラスト調入力では均しすぎる場面がある
- 次の具体化タスクは、青空を含む写真調画像を追加したうえで、青保持と明るい低彩度面の保持を改善する方向に置く
- `DITHER_DIFFUSION_RATE` は固定値の採否だけでなく、スイープや局所差分ベースの非線形制御も follow-up 候補として扱う

## Complexity Tracking

憲章チェック違反なし。named profile と手動差し替え入力画像による比較は実験再現性のための最小限の構造化であり、外部サービスや新規配信経路、自動実験ハーネスを導入する案より単純である。
