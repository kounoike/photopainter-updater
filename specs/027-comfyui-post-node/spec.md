...
- Q: このノードは送信後に画像を次へ流すか → A: 送信専用の終端ノードにして、画像は次へ流さない
- Q: HTTP POST の送信形式は何にするか → A: `Content-Type: image/png` の raw body で送る
- Q: 送信失敗時は workflow 上でどう扱うか → A: node 実行をエラーにして workflow を失敗扱いにする
...
ComfyUI の利用者は workflow 内で生成または加工した画像を、追加の手動保存や別ツールなしで任意の URL へ PNG として POST する終端ノードとして使いたい。

**Why this priority**: この feature の中心価値は、ComfyUI の画像出力をそのまま外部 endpoint へ渡せることにあるため。

**Independent Test**: ComfyUI で画像を生成し、カスタムノードに 026 の `POST /upload` URL を設定して実行したとき、ノードが `Content-Type: image/png` の raw body で送信し、HTTP サーバ側が `200 OK` を返し、更新後画像が反映されることを確認する。

**Acceptance Scenarios**:

1. **Given** 利用者が ComfyUI workflow で画像を扱える状態, **When** 利用者が PNG POST ノードへ画像と送信先 URL を渡して実行する, **Then** ノードは画像を `Content-Type: image/png` の raw body で送信し、送信先から返った成功応答を確認できる終端ノードとして動作する。
...
**Independent Test**: 不正 URL、接続不能 URL、`400` または `500` を返すテスト先を設定し、ノードが実行エラーとして失敗し、利用者が原因を判別できることを確認する。

**Acceptance Scenarios**:

1. **Given** 利用者が到達不能または不正な URL を設定している状態, **When** workflow を実行する, **Then** ノードは送信失敗を成功扱いせず、実行エラーとして停止し、失敗理由を判別できる。
2. **Given** 送信先が失敗 status を返す状態, **When** ノードが画像を送信する, **Then** ノードは実行エラーとして扱い、返却 status を利用者が確認できる。

---

### User Story 3 - 繰り返し使いやすくしたい (Priority: P3)

ComfyUI の利用者は同じ workflow を繰り返し使う際に、送信先 URL や送信結果を終端ノード上で一貫して扱いたい。
...
- 終端ノードとして使うため、送信後に画像を次ノードへ渡す前提を要求しないこと。
- raw body 前提のため、multipart/form-data や独自 envelope を送信先互換のために必須化しないこと。
- URL 不正、接続失敗、失敗 status の各ケースでは、送信結果だけを黙って返して成功扱いにせず、node 実行を失敗として終了すること。
...
- **FR-002a**: System MUST HTTP request body に PNG バイト列をそのまま格納し、`Content-Type: image/png` で送信しなければならない。
...
- **FR-005a**: System MUST URL 不正、接続失敗、失敗 status、入力不足の各ケースで node 実行をエラー終了にし、workflow を失敗扱いにしなければならない。
...
- **FR-009**: System MUST このノードを送信専用の終端ノードとして提供し、送信後の画像出力を必須にしてはならない。

### Key Entities *(include if feature involves data)*

- **送信対象画像**: ComfyUI workflow からノードへ渡される画像。送信時には PNG 表現へ変換される。
- **送信リクエスト**: 送信先 URL、HTTP メソッド `POST`、`Content-Type: image/png` ヘッダ、PNG raw body から構成される単一の送信要求。
- **送信先 URL**: 利用者がノードへ指定する HTTP 送信先。任意 URL を取りうるが、送信可能な URL である必要がある。
- **送信結果**: HTTP status、応答本文要約、成功/失敗判定を含むノード出力または表示情報。終端ノードの実行結果として扱う。
...
- **SC-003a**: 送信失敗時、workflow 実行結果は成功扱いにならず、当該 node が失敗したことを利用者が即座に識別できる。
...
- 026 の `POST /upload` 互換を優先し、送信形式は `Content-Type: image/png` の raw body に固定する。
- ノードは単一画像送信を対象とし、複数画像一括送信や非同期ジョブ管理までは必須範囲に含めない。
- ノードは workflow の途中で画像を受け取り、送信結果だけを返す終端用途を想定する。
- 送信失敗は運用上のエラーとして扱い、workflow 上でも失敗を見逃さないことを優先する。
...
- 送信失敗時に node がエラー終了する前提を利用者向けに記載する必要がある。
