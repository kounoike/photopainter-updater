...
- **FR-004**: System MUST `${COMFYUI_DATA_DIR:-./comfyui-data}` 配下の既存 model、output、input、user 設定の導線を壊してはならない。
- **FR-005**: System MUST 利用者が repo 側 custom node 更新時に再 build が必要かどうかを README または quickstart から判断できるようにしなければならない。
- **FR-006**: System MUST このリポジトリで管理する custom node は volume mount せず image へ焼き込み、追加 custom node は維持対象外である方針を明示しなければならない。
