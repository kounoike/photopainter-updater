...
既存の Rust HTTP サーバへアクセスログを追加し、`/`、`/image.bmp`、失敗系、存在しない path へのアクセスを 1 リクエスト 1 行で追えるようにする。ログは標準出力を主とする既存の起動導線に載せ、少なくとも時刻、アクセス元、対象 path、応答ステータスを含める。既存の BMP 配信内容や route 契約は変更せず、運用時の切り分けしやすさだけを高める。
...
**Constraints**: 既存 route `/` と `/image.bmp` を維持する、ログは標準出力を主とする既存の起動導線で確認できること、外部ログ基盤や永続ログ保管は追加しない  
