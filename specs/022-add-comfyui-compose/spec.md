...
GPU 搭載環境の開発者が、ComfyUI での画像生成時に NVIDIA GPU を活用して高速に処理できる。

**Why this priority**: 画像生成の実用速度に直結するが、機能検証自体はまず起動確認が先のため優先度は低い。

**Independent Test**: GPU 搭載マシンで起動し、ComfyUI のシステム情報に GPU が認識されていることを確認。

**Acceptance Scenarios**:

1. **Given** NVIDIA GPU と NVIDIA Container Toolkit がインストール済みの環境で、**When** ComfyUI を起動したとき、**Then** ComfyUI が GPU を認識して利用できる。

---

### Edge Cases

- GPU ドライバ・NVIDIA Container Toolkit が未インストールの場合、コンテナは起動しない（CPU フォールバックは提供しない）。
...
- **FR-002**: ComfyUI の Web UI はホストの全インターフェース（`0.0.0.0`）に公開され、ポートは `.env` ファイルで設定可能でなければならない（デフォルト `18188`）。
...
- **FR-006**: ボリュームのホスト側ディレクトリパスは `.env` ファイルの環境変数で指定・変更できなければならない（bind mount 方式）。
...
- **SC-003**: GPU 搭載環境で ComfyUI が GPU を認識して利用できる。
...
- GPU サポートは NVIDIA GPU + NVIDIA Container Toolkit を必須とする（AMD/Apple Silicon/CPU フォールバックは対象外）。
- `compose.yml` はリポジトリルートに配置する。
- ComfyUI の公式 Docker イメージ（または信頼できる公開イメージ）を使用する。独自ビルドは行わない。**採用イメージ: `yanwk/comfyui-boot:cu128-slim`（CUDA 12.8 / ComfyUI + ComfyUI Manager 付属 / ローカル開発向け設計）。CUDA 12.9 以降のドライバで動作確認済み。**
- HTTP サーバとの Docker Compose 統合は本フィーチャーのスコープ外（将来フィーチャーで対応）。
- ポート番号はデフォルト `18188`（ホスト側）/ `8188`（コンテナ内部）を使用し、`.env` ファイルで変更可能とする。
