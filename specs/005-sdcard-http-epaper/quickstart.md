...
SDカードルートの `config.txt` を読んで WiFi 接続し、起動時と BOOT ボタン押下時に画像取得して e-paper を更新する専用ファームを実装・検証する。
...
3. `firmware/main/config.*` に `/sdcard/config.txt` 読込と `wifi_ssid` / `wifi_password` / `image_url` の必須項目検証を実装する。
...
1. 正常系 1: SDカードルートに正しい `config.txt` を置いて起動し、60 秒以内に画像更新が完了することを確認する。
2. 正常系 2: 起動後に BOOT ボタンを押し、60 秒以内に画像再取得と表示更新が完了することを確認する。
3. 失敗系 1: `config.txt` を欠落させ、更新処理が失敗理由を判断できる形で終了し、シャットダウンすることを確認する。
