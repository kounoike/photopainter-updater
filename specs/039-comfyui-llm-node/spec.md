...
**Independent Test**: ComfyUI 起動後に追加ノードを workflow へ配置し、`transformers` または `llama-cpp` を選択して `system_prompt` と `user_prompt` を渡したとき、単発のローカル推論が成功し、生成結果文字列を後続ノードで参照できることを確認する。

**Acceptance Scenarios**:

1. **Given** 利用者が ComfyUI 上で local LLM node を利用できる状態, **When** `system_prompt`、`user_prompt`、backend、model 指定を与えて実行する, **Then** node は Python 内でローカル推論を実行し、生成結果を workflow 内で利用できる文字列として返す。
2. **Given** 利用者が同じ workflow を異なる model 指定で再実行する状態, **When** model を切り替えて node を実行する, **Then** node は指定された model を使って独立に推論し、model 切替以外の workflow 構成を変更させない。
...
1. **Given** 利用者が `json_output=true` を設定している状態, **When** node が JSON として解釈可能な応答を生成する, **Then** node はその結果を JSON 文字列として返し、workflow で再利用できる。
2. **Given** 利用者が `json_schema` または `json_schema_path` を設定している状態, **When** node が schema に一致する JSON を生成する, **Then** node は schema 検証済みの結果として成功扱いにする。
...
- `json_output=false` のときは schema 文字列や schema path が与えられていても検証を要求しないこと。
- `json_schema` が空で `json_schema_path` だけが与えられた場合は、指定 path の読込失敗を推論前エラーとして扱うこと。
- `json_schema` と `json_schema_path` が両方与えられた場合は、node 内で優先順位が一意に定義されていること。
- schema 自体が不正な JSON または不正な schema 形式である場合は、推論前に設定不備として失敗すること。
- backend が `think` 設定に対応しない場合でも、未対応設定を黙って別意味へ変換しないこと。
- model の保存先ディレクトリ環境変数が未設定または無効な場合は、利用者が保存先設定不備を判別できること。
...
- **FR-004**: Users MUST be able to Hugging Face 上の model 識別子、または同等に利用者が指定する model 名を node へ与え、backend が解決可能な範囲で切り替えられなければならない。
- **FR-005**: System MUST model 保存先ディレクトリを環境変数で指定でき、その設定を local model 解決に反映しなければならない。
- **FR-006**: System MUST `think` に相当する推論モードを単一 boolean ではなく複数の形式候補から選択できるようにし、backend ごとの対応範囲を越える変換を行ってはならない。
...
- **FR-009**: System MUST `json_schema_path` を任意入力として受け取れるようにし、`json_output=true` かつ `json_schema` が空のときだけ schema 読込元として利用しなければならない。
- **FR-010**: System MUST `json_output=true` かつ schema が指定されている場合、JSON parse と schema 検証の両方に成功した結果だけを成功扱いにしなければならない。
- **FR-011**: System MUST parse 失敗または schema 不一致の場合に限り、設定された上限回数まで再試行できなければならない。
- **FR-012**: System MUST model load 失敗、backend 実行失敗、設定不備、JSON parse 失敗、schema 不一致を区別できる形で利用者へ示さなければならない。
- **FR-013**: System MUST 最終成功時に workflow 内で再利用できる生成結果文字列を返し、必要に応じて JSON 文字列も後続ノードへ渡せるようにしなければならない。
- **FR-014**: System MUST node 内に prompt planning 固有ロジック、画像生成ロジック、長期会話保持、無制限自動修正ループを持ち込んではならない。
- **FR-015**: System MUST 利用者向け文書に backend 選択、model 指定、保存先環境変数、JSON schema 指定、retry 挙動を説明しなければならない。

### Key Entities *(include if feature involves data)*

- **Local LLM Node 設定**: backend、model 識別子、`system_prompt`、`user_prompt`、`think` 形式、`json_output`、schema 指定、retry 回数など、1 回の node 実行条件を表す設定集合。
- **Model 保存先ディレクトリ**: 環境変数で指定される local model cache または保存先。backend が model 解決時に参照する。
- **JSON 出力契約**: `json_output` の有無、`json_schema`、`json_schema_path`、retry 上限、検証成否から成る構造化出力条件。
- **推論結果**: node が返す最終文字列または JSON 文字列。成功時は workflow で再利用され、失敗時は原因区分を伴う実行エラーとして扱われる。
...
- **SC-001**: 利用者は ComfyUI workflow から 1 回の node 実行で local LLM 推論を行い、生成結果を後続ノードへ渡せる。
...
- 利用者は推論対象 model の識別子を把握しており、backend が要求する形式でロード可能な model を選ぶ。
- model 保存先ディレクトリは環境変数で node 実行環境に注入できる。
...
- `comfyui/custom_node/comfyui-photopainter-custom/README.md` に新 node の入出力、backend 選択、model 指定、保存先環境変数を追記する必要がある。
