...
- 選択した backend または model 経路で構造化出力制約を適用できない場合は、暗黙の自由文 fallback ではなく明示 failure とすること。
...
- **FR-009b**: System MUST 選択された backend または model 解決経路で構造化出力制約を満たせない場合、自由文 fallback で成功扱いにせず、明示 failure として利用者へ示さなければならない。
