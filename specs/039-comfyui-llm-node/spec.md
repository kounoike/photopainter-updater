...
- Q: 初期対応する `think_mode` はどこまでか → A: `off`、`qwen`、`gemma`、`deepseek_r1` まで
...
- `think_mode` の初期対応範囲は `off`、`qwen`、`gemma`、`deepseek_r1` に限定し、それ以外を暗黙対応しないこと。
- model の保存先ディレクトリ環境変数が未設定の場合は backend 既定保存先を使い、無効な値が与えられた場合は利用者が保存先設定不備を判別できること。
...
- **FR-005**: System MUST model 保存先ディレクトリを環境変数で任意指定でき、その設定がある場合は local model 解決に反映し、未設定時は backend 既定保存先を使わなければならない。
- **FR-006**: System MUST `think` に相当する推論モードを node 共通入力 `think_mode` の列挙値として選択できるようにし、backend ごとの対応範囲を越える変換を行ってはならない。
- **FR-006a**: System MUST `think_mode` の初期対応値を `off`、`qwen`、`gemma`、`deepseek_r1` に限定しなければならない。
...
- **FR-012**: System MUST 最終成功時に workflow 内で再利用できる生成結果文字列を返し、必要に応じて JSON 文字列も後続ノードへ渡せるようにしなければならない。
...
- **Local LLM Node 設定**: backend、model 識別子、`system_prompt`、`user_prompt`、`think_mode`、`json_output`、schema 指定、retry 回数など、1 回の node 実行条件を表す設定集合。`think_mode` の初期対応値は `off`、`qwen`、`gemma`、`deepseek_r1` とする。
- **Model 保存先ディレクトリ**: 環境変数で任意指定される local model cache または保存先。未設定時は backend 既定保存先を使い、設定がある場合は backend が model 解決時に参照する。
...
- `think_mode` の初期対応を `off`、`qwen`、`gemma`、`deepseek_r1` に限定した最小実装
...
- model 保存先ディレクトリは必要に応じて環境変数で node 実行環境に注入できる。
