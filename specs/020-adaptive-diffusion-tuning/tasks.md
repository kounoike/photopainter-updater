...
**Purpose**: 全 user story に共通する profile・fixture・回帰確認の土台を整える
...
**Goal**: 新しい写真調向け profile を server 側で実装し、既存上位候補と比較できる状態にする

**Independent Test**: 新 profile を選んでローカル変換を実行し、既存上位候補との違いをテストと手動比較の両方で確認できること
...
**Goal**: 比較結果から新 profile の採否と次アクションを判断できる状態にする

**Independent Test**: `specs/020-adaptive-diffusion-tuning/` の成果物を読むことで、新 profile を `advance` / `hold` / `reject` のどれに置くか説明できること
...
- 実機主判定は維持するが、今回の feature ではローカル比較と文書記録も完了条件に含める
