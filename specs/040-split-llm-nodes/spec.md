...
**Independent Test**: `transformers` 専用ノードを workflow に配置し、`model_id`、`think_mode`、`quantization_mode`、`json_output` を指定して実行したとき、量子化設定と think 制御を debug 出力で確認しつつ、最終出力を後続ノードへ渡せることを確認する。
...
ComfyUI の利用者は、`llama-cpp` backend を使う local LLM 推論を別ノードとして利用し、GGUF repo と `model_file` を前提にした設定だけを扱いたい。これにより、`transformers` 向けの量子化や loader 条件と混ざらず、GGUF 運用を単純化したい。
...
- 既存の単一 `PhotoPainter LLM Generate` ノードを即時削除するか互換ラッパーとして残すかは、移行方針を文書で明示し、利用者が突然 workflow を壊されないようにすること。
- `transformers` 専用ノードに `model_file` や GGUF 前提の UI を残さないこと。
...
- **FR-003**: System MUST `llama-cpp` 専用ノードに GGUF 運用向けの `model_file` 入力を持たせ、`transformers` 専用の量子化 UI を露出してはならない。
...
- **FR-006**: System MUST `llama-cpp` 専用ノードで GGUF / `model_file` / context window 前提の validation を担い、`transformers` 側の loader 制約と混在させてはならない。
...
- **FR-009**: System MUST 既存 workflow 互換性への影響を文書で明示し、移行方針を README または関連文書に記載しなければならない。
- **FR-010**: System MUST 画像生成本体の VRAM を優先する前提を維持し、両専用ノードとも生成後の backend メモリ解放方針を崩してはならない。

### Key Entities *(include if feature involves data)*

- **Transformers LLM Node**: `transformers` backend 専用の ComfyUI custom node。`model_id`、`quantization_mode`、family ごとの think 制御、structured output 契約を持つ。
- **LlamaCpp LLM Node**: `llama-cpp` backend 専用の ComfyUI custom node。GGUF 前提の `model_id`、`model_file`、context window validation、structured output 契約を持つ。
...
- `comfyui/custom_node/comfyui-photopainter-custom/README.md` に 2 種類の専用ノード、移行方針、backend 別 UI を追記する必要がある。
